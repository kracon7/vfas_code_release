...
def init_net(opt, net, init_type, init_gain, device):
    if torch.cuda.is_available():
        print(f"CUDA device count = {torch.cuda.device_count()}")
        print(f"Let's use GPUs with IDs: {opt.gpu_ids} !")
...
def define_network(opt, init_type, init_gain, device):
    ...
    return init_net(opt, net, init_type, init_gain, device)


def define_loss(opt):
    return losses.classification_BCE_with_logits
...
class GraspEvaluatorNetwork(nn.Module):
    ...
    def forward(self, pc, pc_features, train=True):
        # print("\tIn Model: input shape", pc.shape)
