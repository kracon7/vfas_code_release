...
def vassilis_loss(pred_logit, gt_label, device):
    positive_grasp_mask = torch.gt(gt_label,0)[:, None]    #Create bool mask from 0s and 1s
    positive_logits = pred_logit[positive_grasp_mask.squeeze()]
    negative_logits = pred_logit[~positive_grasp_mask.squeeze()]
    L_pos = torch.sum(torch.abs(positive_logits))
    L_neg = torch.sum(torch.exp(-torch.abs(negative_logits)))
    return L_pos+L_neg
